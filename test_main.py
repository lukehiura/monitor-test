...
from main import app, Item # make sure to replace 'main' with your actual file name if different
...
def test_create_item():
    ...
    response = client.post("/items/",
        json={"name": "Foo", "price": 123.0, "quantity": 3})
...
def test_create_item_already_exists():
    ...
    client.post("/items/",
        json={"name": "Foo", "price": 123.0, "quantity": 3})
    response = client.post("/items/",
        json={"name": "Foo", "price": 456.0, "quantity": 4})
    assert response.status_code == 400

def test_read_item():
    # Test reading an item
    client.post("/items/",
        json={"name": "Bar", "price": 123.0, "quantity": 3})
